...
export default function HistoryModule() {
  ...
  const filtered = quotes.filter(q =>
    q.product_code.includes(search) || q.product_name.includes(search) || (q.customer || '').includes(search)
  )
    ...
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <div className="flex items-center gap-3 mb-4">
          <input className="input w-64" placeholder="产品编码/名称/客户" value={search} onChange={e => setSearch(e.target.value)} />
          <button className="btn btn-primary" onClick={loadQuotes} disabled={loading}>
            {loading ? '加载中...' : '🔍 搜索'}
          </button>
        ...
        <table className="w-full text-sm">
          <thead><tr className="border-b"><th>报价单号</th><th>产品编码</th><th>产品名称</th><th>客户</th><th>产量</th><th>单位成本</th><th>报价</th><th>毛利率</th><th>创建时间</th></tr></thead>
          <tbody>
            {filtered.map(q => (
