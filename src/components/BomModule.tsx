...
export default function BomModule() {
  ...
  const [calcSemi, setCalcSemi] = useState('SF-001')
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button className="btn btn-secondary btn-sm" onClick={() => setSemiProducts([...semiProducts, { id: Date.now().toString(), code: '', name: '', spec: '', type: '外购', processFee: 0 }])}>+ 新增半成品</button>
            <button className="btn btn-secondary btn-sm" onClick={() => setRawMaterials([...rawMaterials, { id: Date.now().toString(), code: '', name: '', spec: '', unit: '个', price: 0, supplier: '' }])}>+ 新增原材料</button>
            ...
            <tbody>
              {semiProducts.map((s, i) => (
                <tr key={s.id} className="border-b">
                  <td className="py-1"><input className="input w-24 text-sm py-1" value={s.code} onChange={e => { const n = [...semiProducts]; n[i].code = e.target.value; setSemiProducts(n) }} /></td>
                  <td><input className="input w-24 text-sm py-1" value={s.name} onChange={e => { const n = [...semiProducts]; n[i].name = e.target.value; setSemiProducts(n) }} /></td>
                  <td><input className="input w-24 text-sm py-1" value={s.spec} onChange={e => { const n = [...semiProducts]; n[i].spec = e.target.value; setSemiProducts(n) }} /></td>
                  <td><select className="input text-sm py-1" value={s.type} onChange={e => { const n = [...semiProducts]; n[i].type = e.target.value; setSemiProducts(n) }}><option>外购</option><option>自制</option></select></td>
                  <td><input type="number" className="input w-20 text-sm py-1" value={s.processFee} onChange={e => { const n = [...semiProducts]; n[i].processFee = Number(e.target.value); setSemiProducts(n) }} /></td>
                  <td><button className="text-red-500 text-sm" onClick={() => setSemiProducts(semiProducts.filter((_, idx) => idx !== i))}>删除</button></td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {rawMaterials.map((m, i) => (
                <tr key={m.id} className="border-b">
                  <td className="py-1"><input className="input w-24 text-sm py-1" value={m.code} onChange={e => { const n = [...rawMaterials]; n[i].code = e.target.value; setRawMaterials(n) }} /></td>
                  <td><input className="input w-24 text-sm py-1" value={m.name} onChange={e => { const n = [...rawMaterials]; n[i].name = e.target.value; setRawMaterials(n) }} /></td>
                  <td><input className="input w-24 text-sm py-1" value={m.spec} onChange={e => { const n = [...rawMaterials]; n[i].spec = e.target.value; setRawMaterials(n) }} /></td>
                  ...
                  <td><input type="number" className="input w-20 text-sm py-1" value={m.price} onChange={e => { const n = [...rawMaterials]; n[i].price = Number(e.target.value); setRawMaterials(n) }} /></td>
                  ...
                  <td><button className="text-red-500 text-sm" onClick={() => setRawMaterials(rawMaterials.filter((_, idx) => idx !== i))}>删除</button></td>
                </tr>
              ))}
            </tbody>
          </table>
